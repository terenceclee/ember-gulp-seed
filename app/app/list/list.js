...
.controller('SliderCtrl',['$scope', '$http', '$modal', 'settings', function($scope, $http, $modal, settings){
	$scope.$watch("questionId", function(){
		$http.get( settings.basePath + '/service/getAnswers/'+$scope.questionId).success(addSlides);
	});
		...
		var modalInstance = $modal.open({
			templateUrl: settings.basePath + '/assets/javascripts/app/list/partials/form-createAnswer.html',
...
.controller('AnswerCtrl',['$scope', '$http', '$modalInstance', '$fileUploader', 'questionId', 'settings', function($scope, $http, $modalInstance, $fileUploader, questionId, settings){
  ...
  $scope.ok = function ( ) {
  $http.get( settings.basePath + '/service/postAnswers?questionId='+$scope.questionId +
                                          ...
                                          '&firstAnswerImage=' + $scope.answerForm.firstAnswerImage+
                                          '&secondAnswerImage=' + $scope.answerForm.secondAnswerImage+
                                          '&thirdAnswerImage=' + $scope.answerForm.thirdAnswerImage
             ).success(completed);
  };

  completed = function(data){
    $modalInstance.close();
    window.location.href= settings.basePath + '/question/' + data.list.questionId;
  }

  // create a uploader with options
  var uploader = $scope.uploader = $fileUploader.create({
    scope: $scope,                          // to automatically update the html. Default: $rootScope
    url: settings.imageServiceHost + settings.imageServicePath + '/content/image/create',
    autoUpload: true,
    formData: [
      { key: 'value',
...
.controller('CreateCtrl', ['$scope', '$modal', 'settings', function ($scope, $modal, settings){
  $scope.ask = function(){
  	$scope.modalInstance = $modal.open({
                        templateUrl:  settings.basePath + '/assets/javascripts/app/list/partials/form-createList.html',
...
.controller('QuestionCtrl', ['$scope', '$http', '$modal', '$modalInstance', 'settings',  function($scope, $http, $modal, $modalInstance, settings){
  ...
  $http.get( settings.basePath + '/service/getCategories').success(processCategories);
  ...
  $scope.next = function (){
    $modalInstance.close();
    $http.get( settings.basePath + '/service/postQuestion?categoryId='+$scope.questionForm.categoryId+"&questionText="+$scope.questionForm.questionType + " " + $scope.questionForm.questionText).success(openAnswerForm);
  };
